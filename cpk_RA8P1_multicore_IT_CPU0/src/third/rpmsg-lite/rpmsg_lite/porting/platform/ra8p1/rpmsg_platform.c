...
// RX callback for CPU0 (IPC0 CH0 receives from CPU1)
void g_ipc0_callback(ipc_callback_args_t *p_args)
{
    /* Invalidate shared memory cache before reading vring data from CPU1 */
    SCB_InvalidateDCache_by_Addr((volatile void *)0x220E2000, 64 * 1024);
...
int32_t platform_init_interrupt(uint32_t vector_id, void *isr_data)
{
    if (platform_lock != ((void *)0))
    {
        ...
        if (isr_counter == 0) NVIC_SetPriority(IPC_IRQ0_IRQn, 10);
        isr_counter++;
...
int32_t platform_deinit_interrupt(uint32_t vector_id)
{
    if (platform_lock != ((void *)0))
    {
        ...
        RL_ASSERT(0 < isr_counter);
        isr_counter--;
        if (isr_counter == 0) NVIC_DisableIRQ(IPC_IRQ0_IRQn);
...
void platform_notify(uint32_t vector_id)
{
    ...
    /* Flush shared memory so CPU1 sees the vring data */
    SCB_CleanDCache_by_Addr((volatile void *)0x220E2000, 64 * 1024);

    /* EventGenerate triggers CPU1 interrupt (MessageSend alone doesn't work on RA8P1) */
    R_IPC_EventGenerate(&g_ipc1_ctrl, (ipc_generate_event_t)(1U << msg));
...
int32_t platform_interrupt_enable(uint32_t vector_id)
{
    ...
    disable_counter--;
    if (disable_counter == 0) NVIC_EnableIRQ(IPC_IRQ0_IRQn);
...
int32_t platform_interrupt_disable(uint32_t vector_id)
{
    RL_ASSERT(0 <= disable_counter);
    platform_global_isr_disable();
    if (disable_counter == 0) NVIC_DisableIRQ(IPC_IRQ0_IRQn);
    disable_counter++;
    platform_global_isr_enable();
    return ((int32_t)vector_id);
}
